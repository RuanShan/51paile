# This migration comes from spree (originally 20150314013438)
class AddMissingIndexesOnSpreeTables < ActiveRecord::Migration[4.2]
  def change
    unless index_exists? :spree_promotion_rule_users,
                         [:user_id, :promotion_rule_id],
                         name: 'index_promotion_rules_users_on_user_id_and_promotion_rule_id'
      add_index :spree_promotion_rule_users,
                [:user_id, :promotion_rule_id],
                name: 'index_promotion_rules_users_on_user_id_and_promotion_rule_id'
    end

    unless index_exists? :spree_product_promotion_rules,
                         [:promotion_rule_id, :product_id],
                         name: 'index_products_promotion_rules_on_promotion_rule_and_product'
      add_index :spree_product_promotion_rules,
                [:promotion_rule_id, :product_id],
                name: 'index_products_promotion_rules_on_promotion_rule_and_product'
    end

    unless index_exists? :spree_orders, :canceler_id
      add_index :spree_orders, :canceler_id
    end

    unless index_exists? :spree_orders, :store_id
      add_index :spree_orders, :store_id
    end

    unless index_exists? :spree_order_promotions, [:promotion_id, :order_id]
      add_index :spree_order_promotions, [:promotion_id, :order_id]
    end

    unless index_exists? :spree_property_prototypes, :prototype_id
      add_index :spree_property_prototypes, :prototype_id
    end

    unless index_exists? :spree_property_prototypes,
                         [:prototype_id, :property_id],
                         name:  'index_properties_prototypes_on_prototype_and_property'
      add_index :spree_property_prototypes,
                [:prototype_id, :property_id],
                name:  'index_properties_prototypes_on_prototype_and_property'
    end

    unless index_exists? :spree_prototype_taxons, [:prototype_id, :taxon_id]
      add_index :spree_prototype_taxons, [:prototype_id, :taxon_id]
    end

    unless index_exists? :spree_option_type_prototypes, :prototype_id
      add_index :spree_option_type_prototypes, :prototype_id
    end

    unless index_exists? :spree_option_type_prototypes,
                         [:prototype_id, :option_type_id],
                         name: 'index_option_types_prototypes_on_prototype_and_option_type'
      add_index :spree_option_type_prototypes,
                [:prototype_id, :option_type_id],
                name: 'index_option_types_prototypes_on_prototype_and_option_type'
    end

    unless index_exists? :spree_option_value_variants,
                         [:option_value_id, :variant_id],
                         name: 'index_option_values_variants_on_option_value_and_variant'
      add_index :spree_option_value_variants,
                [:option_value_id, :variant_id],
                name: 'index_option_values_variants_on_option_value_and_variant'
    end
  end
end
